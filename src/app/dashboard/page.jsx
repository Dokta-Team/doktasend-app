  ...
  useEffect(() => {
    const fetchRecipients = async () => {
      ...
      if (response.ok) {
        ...
        console.error('Failed to fetch recipients from dashboard API');
      }
    };
    fetchRecipients();
  }, []);
            ...
            <div className="space-y-4">
              {recipients.length === 0 ? (
                <div className="text-center p-6">
                  <p className="text-sm text-muted-foreground mb-4">You have no active recipients.</p>
                ...
                recipients.map((recipient) => (
                  <div key={recipient._id} className="flex items-center justify-between p-4 border rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300">
                    ...
                    <Button
                      variant="outline"
                      onClick={() => setActiveRecipient(recipient.name)}
